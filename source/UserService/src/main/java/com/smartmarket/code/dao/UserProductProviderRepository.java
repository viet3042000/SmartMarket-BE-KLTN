...
public interface UserProductProviderRepository extends JpaRepository<UserProductProvider, String> {
    //check lại nếu có nhiều UserProductProvider
    @Query(value = "Select * FROM user_product_provider where product_provider_id =:product_provider_id", nativeQuery = true)
    public List<UserProductProvider> findByProductProviderId(@Param("product_provider_id") Long productProviderId) ;
