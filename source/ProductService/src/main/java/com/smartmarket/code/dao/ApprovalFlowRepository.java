...
public interface ApprovalFlowRepository extends JpaRepository<ApprovalFlow, Long> {
    ...
    @Query(value = "DELETE FROM approval_flow where id =:id", nativeQuery = true)
    public int deleteApprovalFlowKafka(@Param("id") Long id) ;

    @Modifying(clearAutomatically = true)
    @Query(value = "TRUNCATE TABLE approval_flow",  nativeQuery = true)
    public int truncateApprovalFlowKafka() ;
}
